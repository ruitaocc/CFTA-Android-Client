...
public class DataUtil {
	...
	//将rs中的值封装成一个集合
	public static Collection<Object> getDatas(Collection<Object> result, Cursor cursor, Class<?> clazz) {
		try {
			cursor.moveToFirst();
			for (int i = 0; i<cursor.getCount();i++) {
				...
				for (Field field : allFields) {
					...
					invokeMethod(cursor, field, vo, setterMethod);
				}
				result.add(vo);
				cursor.moveToNext();
			}
	...
	//执行一个方法, 从Cursor中获取一个字段的数据, 调用vo的setter方法
	private static void invokeMethod(Cursor cursor, Field field, Object vo, 
			Method setterMethod) {
		try {
